...
exports['match'] = {
  'empty set': function(test) {
    ...
    test.deepEqual(globule.match(null, 'foo.src'), [], 'should return empty set.');
    test.deepEqual(globule.match('*.src', null), [], 'should return empty set.');
    test.deepEqual(globule.match([], 'foo.src'), [], 'should return empty set.');
    test.deepEqual(globule.match('*.src', []), [], 'should return empty set.');
    test.deepEqual(globule.match(null, ['foo.src']), [], 'should return empty set.');
    test.deepEqual(globule.match(['*.src'], null), [], 'should return empty set.');
    test.done();
  },
  'basic matching': function(test) {
    test.expect(6);
    test.deepEqual(globule.match('*.src', 'foo.src'), ['foo.src'], 'should match correctly.');
    test.deepEqual(globule.match('*.src', ['foo.src']), ['foo.src'], 'should match correctly.');
    test.deepEqual(globule.match('*.src', ['foo.src', 'bar.css']), ['foo.src'], 'should match correctly.');
    test.deepEqual(globule.match(['*.src', '*.css'], 'foo.src'), ['foo.src'], 'should match correctly.');
    test.deepEqual(globule.match(['*.src', '*.css'], ['foo.src']), ['foo.src'], 'should match correctly.');
    test.deepEqual(globule.match(['*.src', '*.css'], ['foo.src', 'bar.css']), ['foo.src', 'bar.css'], 'should match correctly.');
    test.done();
  },
  'no matches': function(test) {
    test.expect(2);
    test.deepEqual(globule.match('*.src', 'foo.css'), [], 'should fail to match.');
    test.deepEqual(globule.match('*.src', ['foo.css', 'bar.css']), [], 'should fail to match.');
    test.done();
  },
  'unique': function(test) {
    test.expect(2);
    test.deepEqual(globule.match('*.src', ['foo.src', 'foo.src']), ['foo.src'], 'should return a uniqued set.');
    test.deepEqual(globule.match(['*.src', '*.*'], ['foo.src', 'foo.src']), ['foo.src'], 'should return a uniqued set.');
    test.done();
  },
  'flatten': function(test) {
    test.expect(1);
    test.deepEqual(globule.match([['*.src', '*.css'], ['*.*', '*.src']], ['foo.src', 'bar.css']),
      ['foo.src', 'bar.css'],
  ...
  'exclusion': function(test) {
    test.expect(5);
    test.deepEqual(globule.match(['!*.src'], ['foo.src', 'bar.src']), [], 'solitary exclusion should match nothing');
    test.deepEqual(globule.match(['*.src', '!*.src'], ['foo.src', 'bar.src']), [], 'exclusion should cancel match');
    test.deepEqual(globule.match(['*.src', '!f*.src'], ['foo.src', 'bar.src', 'baz.src']),
      ['bar.src', 'baz.src'],
      'partial exclusion should partially cancel match');
    test.deepEqual(globule.match(['*.src', '!*.src', 'b*.src'], ['foo.src', 'bar.src', 'baz.src']),
      ['bar.src', 'baz.src'],
      'inclusion / exclusion order matters');
    test.deepEqual(globule.match(['*.src', '!f*.src', '*.src'], ['foo.src', 'bar.src', 'baz.src']),
      ['bar.src', 'baz.src', 'foo.src'],
      'inclusion / exclusion order matters');
    test.done();
  },
  'options.matchBase': function(test) {
    test.expect(2);
    test.deepEqual(globule.match('*.src', ['foo.src', 'bar', 'baz/xyz.src'], {matchBase: true}),
      ['foo.src', 'baz/xyz.src'],
      'should matchBase (minimatch) when specified.');
    test.deepEqual(globule.match('*.src', ['foo.src', 'bar', 'baz/xyz.src']),
      ['foo.src'],
...
exports['isMatch'] = {
  'basic matching': function(test) {
    test.expect(6);
    test.ok(globule.isMatch('*.src', 'foo.src'), 'should match correctly.');
    test.ok(globule.isMatch('*.src', ['foo.src']), 'should match correctly.');
    test.ok(globule.isMatch('*.src', ['foo.src', 'bar.css']), 'should match correctly.');
    test.ok(globule.isMatch(['*.src', '*.css'], 'foo.src'), 'should match correctly.');
    test.ok(globule.isMatch(['*.src', '*.css'], ['foo.src']), 'should match correctly.');
    test.ok(globule.isMatch(['*.src', '*.css'], ['foo.src', 'bar.css']), 'should match correctly.');
    test.done();
  },
  'no matches': function(test) {
    test.expect(6);
    test.ok(!globule.isMatch('*.src', 'foo.css'), 'should fail to match.');
    test.ok(!globule.isMatch('*.src', ['foo.css', 'bar.css']), 'should fail to match.');
    test.ok(!globule.isMatch(null, 'foo.css'), 'should fail to match.');
    test.ok(!globule.isMatch('*.src', null), 'should fail to match.');
    test.ok(!globule.isMatch([], 'foo.css'), 'should fail to match.');
    test.ok(!globule.isMatch('*.src', []), 'should fail to match.');
    test.done();
  },
  'options.matchBase': function(test) {
    test.expect(2);
    test.ok(globule.isMatch('*.src', ['baz/xyz.src'], {matchBase: true}), 'should matchBase (minimatch) when specified.');
    test.ok(!globule.isMatch('*.src', ['baz/xyz.src']), 'should not matchBase (minimatch) by default.');
    test.done();
  },
};
...
exports['find'] = {
  ...
  'basic matching': function(test) {
    test.expect(5);
    test.deepEqual(globule.find('**/*.src'), ['src/bar.src', 'src/foo.src'], 'single pattern argument should match.');
    test.deepEqual(globule.find('**/*.src', '**/*.css'),
      ['src/bar.src', 'src/foo.src', 'css/baz.css', 'css/qux.css'],
      'multiple pattern arguments should match.');
    test.deepEqual(globule.find(['**/*.src', '**/*.css']),
      ['src/bar.src', 'src/foo.src', 'css/baz.css', 'css/qux.css'],
      'array of patterns should match.');
    test.deepEqual(globule.find([['**/*.src'], [['**/*.css', 'src/*.src']]]),
      ['src/bar.src', 'src/foo.src', 'css/baz.css', 'css/qux.css'],
  ...
  'unique': function(test) {
    test.expect(4);
    test.deepEqual(globule.find('**/*.src', 'src/*.src'),
      ['src/bar.src', 'src/foo.src'],
      'file list should be uniqed.');
    test.deepEqual(globule.find('**/*.src', '**/*.css', 'src/*.src'), ['src/bar.src', 'src/foo.src',
      'css/baz.css', 'css/qux.css'],
      'file list should be uniqed.');
    test.deepEqual(globule.find('src', 'src/'),
      ['src', 'src/'],
      'mixed non-ending-/ and ending-/ dirs will not be uniqed by default.');
    test.deepEqual(globule.find('src', 'src/', {mark: true}),
      ['src/'],
  ...
  'file order': function(test) {
    test.expect(5);
    var actual = globule.find('**/*.{src,css}');
    var expected = ['css/baz.css', 'css/qux.css', 'src/bar.src', 'src/foo.src'];
    test.deepEqual(actual, expected, 'should select 4 files in this order, by default.');

    actual = globule.find('src/foo.src', 'src/bar.src', '**/*.{src,css}');
    expected = ['src/foo.src', 'src/bar.src', 'css/baz.css', 'css/qux.css'];
    test.deepEqual(actual, expected, 'specifically-specified-up-front file order should be maintained.');

    actual = globule.find('src/bar.src', 'src/foo.src', '**/*.{src,css}');
    expected = ['src/bar.src', 'src/foo.src', 'css/baz.css', 'css/qux.css'];
    test.deepEqual(actual, expected, 'specifically-specified-up-front file order should be maintained.');

    actual = globule.find('**/*.{src,css}', '!css/qux.css', 'css/qux.css');
    expected = ['css/baz.css', 'src/bar.src', 'src/foo.src', 'css/qux.css'];
    test.deepEqual(actual, expected, 'if a file is excluded and then re-added, it should be added at the end.');

    actual = globule.find('src/foo.src', '**/*.{src,css}', '!css/qux.css', 'css/qux.css');
    expected = ['src/foo.src', 'css/baz.css', 'src/bar.src', 'css/qux.css'];
    test.deepEqual(actual, expected, 'should be able to combine specified-up-front and excluded/added-at-end.');
    test.done();
  },
  'exclusion': function(test) {
    test.expect(8);
    test.deepEqual(globule.find(['!src/*.src']), [], 'solitary exclusion should match nothing');
    test.deepEqual(globule.find(['src/bar.src','!src/bar.src']), [], 'exclusion should negate match');
    test.deepEqual(globule.find(['**/*.src', '!src/foo.src']),
      ['src/bar.src'],
      'should omit single file from matched set');
    test.deepEqual(globule.find(['!src/foo.src', '**/*.src']),
      ['src/bar.src', 'src/foo.src'],
      'inclusion / exclusion order matters');
    test.deepEqual(globule.find(['**/*.src', '**/*.css', '!src/bar.src', '!css/baz.css']),
      ['src/foo.src','css/qux.css'],
      'multiple exclusions should be removed from the set');
    test.deepEqual(globule.find(['**/*.src', '**/*.css', '!**/*.css']),
      ['src/bar.src', 'src/foo.src'],
      'excluded wildcards should be removed from the matched set');
    test.deepEqual(globule.find(['src/bar.src', 'src/foo.src', 'css/baz.css', 'css/qux.css', '!**/b*.*']),
      ['src/foo.src', 'css/qux.css'],
      'different pattern for exclusion should still work');
    test.deepEqual(globule.find(['src/bar.src', '!**/b*.*', 'src/foo.src', 'css/baz.css', 'css/qux.css']),
      ['src/foo.src', 'css/baz.css', 'css/qux.css'],
      'inclusion / exclusion order matters');
    test.done();
  },
  ...
  'options.filter': function(test) {
    ...
    test.deepEqual(globule.find('src', 'css', {filter: 'isFile'}), [], 'should fail to match.');
    test.deepEqual(globule.find('**/*.src', {filter: 'isDirectory'}), [], 'should fail to match.');
    test.done();
  },
  'options.matchBase': function(test) {
    test.expect(3);
    test.deepEqual(globule.find('*.src'), [], 'should not matchBase (minimatch) by default.');
    test.deepEqual(globule.find('*.src', {matchBase: true}),
      ['src/bar.src', 'src/foo.src'],
      'matchBase option should be passed through to minimatch.');
    test.deepEqual(globule.find('*.src', '*.css', {matchBase: true}),
      ['src/bar.src', 'src/foo.src', 'css/baz.css', 'css/qux.css'],
      'matchBase option should be passed through to minimatch.');
    test.done();
  },
  ...
  'options.nonull': function(test) {
    ...
    test.deepEqual(globule.find(['src/a*', 'src/b*', 'src/c*'], {nonull: true}),
      ['src/a*', 'src/bar.src', 'src/c*'],
      'non-matching patterns should be returned in result set.');
    test.deepEqual(globule.find(['src/foo.src', 'src/bar.src', 'src/nonexistent.src'], {nonull: true}),
      ['src/foo.src', 'src/bar.src', 'src/nonexistent.src'],
...
exports['mapping'] = {
  ...
  'options.ext': function(test) {
    ...
    actual = globule.mapping(['x/a.src', 'x.y/b.min.src', 'x.y/z.z/c'], {ext: '.foo'});
    expected = [
      {dest: 'x/a.foo', src: ['x/a.src']},
      {dest: 'x.y/b.foo', src: ['x.y/b.min.src']},
  ...
  'options.extDot': function(test) {
    ...
    actual = globule.mapping(['x/a.src', 'x.y/b.bbb.min.src', 'x.y/z.z/c'], {ext: '.foo', extDot: 'first'});
    expected = [
      {dest: 'x/a.foo', src: ['x/a.src']},
      {dest: 'x.y/b.foo', src: ['x.y/b.bbb.min.src']},
      {dest: 'x.y/z.z/c.foo', src: ['x.y/z.z/c']},
    ];
    test.deepEqual(actual, expected, 'extDot of "first" should replace everything after the first dot in the filename.');

    actual = globule.mapping(['x/a.src', 'x.y/b.bbb.min.src', 'x.y/z.z/c'], {ext: '.foo', extDot: 'last'});
    expected = [
      {dest: 'x/a.foo', src: ['x/a.src']},
      {dest: 'x.y/b.bbb.min.foo', src: ['x.y/b.bbb.min.src']},
      {dest: 'x.y/z.z/c.foo', src: ['x.y/z.z/c']},
    ];
